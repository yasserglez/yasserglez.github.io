...
responsively use `figure` with the appropriate `class`. Each instance
of `figure` is auto-numbered and displayed in the caption.
...
Apply the `half` class like so to display two images side by side that
...
Apply the `third` class like so to display three images side by side
